  ...
  const onDelete = async (employee) => {
    try {
      ...
      const res = await axios.delete(
        `${process.env.NEXT_PUBLIC_API_URL}/employees/delete/${employee["_id"]}`,
        {
    ...
    try {
      ...
      if (user == null) {
        ...
        const res = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/employees/paging/${page}/10`,
          {
            signal: controller.signal,
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        if (res.status == 200) {
          ...
          setData({
            ...
            headers: ["nik", "name", "address", "phone", "email"],
            range: range,
          })
        }
      }
